...
Hover or tap a skin-type label for a healthy-volunteer example whose phototype is documented by a scientific source. Types II, III, V and VI show cheek views from UMINHO-HSFD; types I and IV show forearms from Mantri & Jokerst (2022), Figure 2. The original source images are preserved, with SVG viewports for the Minho closeups. Source labels, capture differences, limited representation and licenses are disclosed in all three languages and documented in `public/photos/skin-types/README.md`. Minho uses CC BY 4.0; the two Optica images permit noncommercial reuse with attribution. These examples do not determine a personal phototype or UV threshold. Chart hover cards stay clear of the pointer and flip at viewport edges. UV Index is linear: a 10% increase turns UVI 3 into 3.3 and UVI 6 into 6.6; surface reflectance is not itself the percentage increase in UVI.
